...
sudo docker run -v ${RESOURCES_DIR}:/root/pipeline/resources -v ${CODE_DIR}:/root/pipeline/code -v ${INPUT_DIR}:/root/input -v ${OUTPUT_DIR}/output:/root/output -v ${SEQ_PIPELINE_DIR}:/root/seq_pipeline wpierson/seq_pipeline:1.0 -it /root/seq_pipeline/aws_pipeline.sh # -I /root/input -O /root -t $THREADS -s $SAMPLE_THREADS -w $WORKERS $LOCAL_SCHEDULER 