...
class realigner_target(luigi.Task):
	...
	def run(self):
		cmd = ['java', '-Djava.io.tmpdir=%s' % self.cfg['tmp_dir'], '-jar', '$GATK3', '-T', 'RealignerTargetCreator', '-R', self.cfg['fasta_file'], '--known', self.cfg['germline_indels'], '-nt', '4', '-o', self.output()['realigner_target'].path] # , '-nct', str(int(self.cfg['global_max_threads']/4))
		file_map = []
